...
""" mrq-worker settings
"""
QUEUES = ("_system", "_stats", "_cleaning", "default", "high/", "monitor", "medium/", "low/", "_heavy") # The queues to listen on.Defaults to default , which will listen on all queues.
MAX_JOBS = 8 #Gevent:max number of jobs to do before quitting. Workaround for memory leaks in your tasks. Defaults to 0
MAX_MEMORY = 10000 #Max memory (in Mb) after which the process will be shut down. Use with PROCESS = [1-N] to have supervisord automatically respawn the worker when this happens.Defaults to 1
GREENLETS = 30 #Max number of greenlets to use.Defaults to 1.
...
SCHEDULER_INTERVAL = 5 #Seconds between scheduler checks.Defaults to 60 seconds, only ints are acceptable.
...
MAX_LATENCY = 0.2  #Max seconds while worker may sleep waiting for a new job.Can be < 1 and a float value.
...
SCHEDULER_TASKS = [
    {"path": "tasks.misc.ScheduledScans", "params": {}, "interval": 3 * 60.0, "queue": "_scheduled_scans"},
    # {"path": "tasks.misc.UpdateScanStatistics", "params": {}, "interval": 3 * 60.0, "queue": "_stats"},
    # {"path": "tasks.misc.UpdateWorkerInstantStatistics", "params": {}, "interval": 5.0, "queue": "_stats_online"},
    # {"path": "tasks.misc.UpdateScanInstantStatistics", "params": {}, "interval": 5.0, "queue": "_stats_online"},
    # {"path": "tasks.misc.FlushDashboardLogs", "params": {}, "interval": 4.0, "queue": "_stats_online"},
    # {"path": "mrq.basetasks.cleaning.RequeueLostJobs", "params": {}, "interval": 6 * 60.0, "queue": "_cleaning"},
    # {"path": "mrq.basetasks.cleaning.RequeueInterruptedJobs", "params": {}, "interval": 3 * 60.0, "queue": "_cleaning"},
    # {"path": "mrq.basetasks.cleaning.RequeueRetryJobs", "params": {}, "interval": 3 * 60.0, "queue": "_cleaning"},
    # {"path": "mrq.basetasks.cleaning.RequeueStartedJobs", "params": {"timeout": 4 * 60.0}, "interval": 7 * 60.0, "queue": "_cleaning"},
    # {"path": "mrq.basetasks.cleaning.RequeueRedisStartedJobs", "params": {}, "interval": 10 * 60.0, "queue": "_cleaning"},
    #{"path": "tasks.RequeueFailedKasirgaJobs", "params": {}, "interval": 5 * 60.0, "queue": "_cleaning"},
    # {"path": "tasks.RestartZombieJobs", "params": {}, "interval": 10 * 60.0, "queue": "_cleaning"},
]


TASKS = {
    "tasks.commands.nmapscan.NmapScan": {
